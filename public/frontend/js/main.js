...
navigator.serviceWorker.register("/firebase-messaging-sw.js").then((registration) => {
  ...
  getToken(messaging, {
    ...
    if (currentToken) {
      ...
      if (!sessionStorage.getItem("fcm_notified")) {
      ...
      .then(() => {
        sessionStorage.setItem("fcm_notified", "1");
      })
      .catch(error => {
      ...
      console.warn("⚠️ No registration token available. User may have denied permission.");
    }
  }).catch((err) => {
    console.error("❌ Error retrieving token:", err);
  });

  // ✅ Show notification if message received while app is in foreground
